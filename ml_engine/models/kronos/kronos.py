...
class Kronos(nn.Module, PyTorchModelHubMixin):
    ...
    def enable_mc_dropout(self):
        """Enable Monte Carlo Dropout during inference by setting dropout modules to train mode."""
...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False):
    with torch.no_grad():
        ...
        preds_raw = z.cpu().numpy()
        preds_mean = np.mean(preds_raw, axis=1)
        preds_std = np.std(preds_raw, axis=1)
...
class KronosPredictor:
    ...
    def generate(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, mc_dropout=False):
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        preds_mean, preds_std = self.generate(x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose, mc_dropout=True)

        preds_mean = preds_mean.squeeze(0)
        preds_std = preds_std.squeeze(0)
        
        preds_mean = preds_mean * (x_std + 1e-5) + x_mean
        preds_std = preds_std * (x_std + 1e-5) # Std scales linearly

        pred_df = pd.DataFrame(preds_mean, columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp)
        std_cols = [f"{c}_std" for c in self.price_cols + [self.vol_col, self.amt_vol]]
        pred_df[std_cols] = preds_std
        return pred_df
    ...
    def predict_batch(self, df_list, x_timestamp_list, y_timestamp_list, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        preds_mean, preds_std = self.generate(x_batch, x_stamp_batch, y_stamp_batch, pred_len, T, top_k, top_p, sample_count, verbose, mc_dropout=True)
        # preds: (B, pred_len, feat)

        pred_dfs = []
        for i in range(num_series):
            preds_mean_i = preds_mean[i] * (stds[i] + 1e-5) + means[i]
            preds_std_i = preds_std[i] * (stds[i] + 1e-5)
            pred_df = pd.DataFrame(preds_mean_i, columns=self.price_cols + [self.vol_col, self.amt_vol], index=y_timestamp_list[i])
            std_cols = [f"{c}_std" for c in self.price_cols + [self.vol_col, self.amt_vol]]
            pred_df[std_cols] = preds_std_i
